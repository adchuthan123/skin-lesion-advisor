...
# ── Config ──────────────────────────────────────────────────────────────────────
HF_MODEL_ID    = os.getenv('HF_MODEL_ID', 'PREMAADC/vit-base-ham10000')
...
vit_classifier = pipeline('image-classification', model=HF_MODEL_ID, image_processor=vit_processor)
...
def classify_openai(image: Image.Image) -> dict:
    if openai_client is None:
        return {'info': 'OpenAI API Key nicht gesetzt.'}
    prompt = (
...
def run_pipeline(image, age, sex, localization):
    ...
    return vit_output, clip_output, openai_str, summary, explanation
...
with gr.Blocks(title='Skin Lesion Risk Advisor') as demo:
    ...
    **ZHAW KI-Anwendungen Projekt** — Multimodales KI-System zur Risikoeinschätzung von Hautläsionen
    ...
    with gr.Row():
        with gr.Column():
            image_input = gr.Image(label='Bild der Hautläsion hochladen', type='numpy')
    ...
    with gr.Row():
        ...
        openai_out = gr.Textbox(label='OpenAI Vision', lines=4)
